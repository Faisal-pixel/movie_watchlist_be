...
router.post('/add-movie-to-watchlist/:watchlist_id', authenticatToken, body("tmdb_movie_id").notEmpty().withMessage("TMDB Movie ID is required"), param("watchlist_id").notEmpty().withMessage("Watchlist ID is required"), async (req: IRequest, res: Response) => {
    ...
    try {
        ...
        const does_movie_exist = await pool.query('SELECT * FROM watchlist_movie WHERE watchlist_id = $1 AND tmdb_movie_id = $2', [watchlist_id, tmdb_movie_id])
        if(does_movie_exist.rows.length > 0) {
            res.status(400).json({success: false, message: "Movie already exist in the watchlist"});
            return;
        }
        ...
        await pool.query('INSERT INTO watchlist_movie (watchlist_id, tmdb_movie_id, added_at) VALUES ($1, $2, $3) ON CONFLICT (watchlist_id, tmdb_movie_id) DO NOTHING', [watchlist_id, tmdb_movie_id, added_at]);

        /**GETTING THE GENRE FROM TMDB */
        const movieGenres: TmovieGenre[] = await getMovieGenres(tmdb_movie_id);
        for (let genre of movieGenres) {
            const response = await pool.query('SELECT * FROM genre WHERE id = $1', [genre.id]);
            if(response.rows.length === 0) {
                await pool.query('INSERT INTO genre (id, name) VALUES ($1, $2) ON CONFLICT DO NOTHING', [genre.id, genre.name]);
            }

            /**INSERTING INTO THE MOVIE_GENRE TABLE */

            await pool.query("INSERT INTO movie_genre (tmdb_movie_id, genre_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [tmdb_movie_id, genre.id]);
        }

        res.status(200).json({success: true, message: "Movie added to watchlist succesfully"});
    } catch (error) {
        if (error instanceof Error) {
            res.status(500).json({success: false, message: "Server error", error: error.message});
            return;
